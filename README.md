...
- activity rhythm metrics for longest active streak, busiest weekday, most active month, average gap, and weekend share
- hikes-versus-trains comparison for distance share, counts, and average route length
- clickable daily activity calendars by year, with hike/train/mixed-day colors and a legend for the active filter/timeframe
...
- oldest-backfill queue with direct edit links for the next incomplete routes to clean up
...
- Add a route name, optional date, and optional notes; date shortcuts support today and one-day back/forward adjustments.
- Add optional tags and duration, with quick duration presets and 15-minute adjustments for common backfill entries. Elevation gain/loss is calculated automatically for hikes: GPX elevation is used locally when available, and clicked routes use an external elevation lookup.
...
2. Improve route editing
   ...
   - Add full-resolution geometry editing for very dense GPX tracks without overwhelming the browser.
