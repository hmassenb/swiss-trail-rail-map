...
Saved route cards can be clicked or focused with the keyboard to zoom the route on the map. Existing route cards can also be duplicated, exported as individual GPX files, or edited, which is useful when backfilling similar hikes or train rides from older years.
